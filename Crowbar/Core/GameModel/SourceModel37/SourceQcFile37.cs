...
#region Private Methods
		...
		public void WriteModelCommand()
		{
			...
			if (theMdlFileData.theBodyParts != null && theMdlFileData.theBodyParts.Count > 0)
			{
				...
				aBodyModel.theSmdFileNames[0] = SourceFileNamesModule.CreateBodyGroupSmdFileName(aBodyModel.theSmdFileNames[0], 0, 0, 0, theModelName, new string(theMdlFileData.theBodyParts[0].theModels[0].name));
		...
		private void WriteEyelidLines(List<string> eyeballNames)
		{
			...
			try
			{
				...
				aBodyPart = theMdlFileData.theBodyParts[0];
				if (aBodyPart.theModels != null && aBodyPart.theModels.Count > 0 && theMdlFileData.theEyelidFlexFrameIndexes.Count > 0)
				{
					aModel = aBodyPart.theModels[0];
					if (aModel.theEyeballs != null && aModel.theEyeballs.Count > 0)
					{
						...
						for (int eyeballIndex = 0; eyeballIndex < aModel.theEyeballs.Count; eyeballIndex++)
						{
							...
							//TEST:
							line += theMdlFileData.theEyelidFlexFrameIndexes[frameIndex].ToString(MainCROWBAR.TheApp.InternalNumberFormat);
							frameIndex += 1;
							line += " ";
...
//INSTANT C# NOTE: There is no C# equivalent to VB's implicit 'once only' variable initialization within loops, so the following variable declaration has been placed prior to the loop:
	...
	//					Dim intermediateExp As IntermediateExpression
		...
		public void WriteIncludeModelCommands()
		{
			...
			if (theMdlFileData.theIncludeModelFileNames != null)
			{
				line = "";
				theOutputFileStreamWriter.WriteLine(line);

				for (int i = 0; i < theMdlFileData.theIncludeModelFileNames.Count; i++)
				{
					string anIncludeModelFileName = theMdlFileData.theIncludeModelFileNames[i];
		...
		private void WriteAnimationOrDeclareAnimationCommand()
		{
			if (theMdlFileData.theAnimationDescs != null)
			{
				...
				{
					SourceMdlAnimationDesc37 anAnimationDesc = theMdlFileData.theAnimationDescs[i];
		...
		private void WriteSequenceOrDeclareSequenceCommand()
		{
			...
			if (theMdlFileData.theSequenceDescs != null)
			{
				for (int i = 0; i < theMdlFileData.theSequenceDescs.Count; i++)
				{
					SourceMdlSequenceDesc37 aSequenceDesc = theMdlFileData.theSequenceDescs[i];
		...
		private void WriteSequenceOptions(SourceMdlSequenceDesc37 aSequenceDesc)
		{
			...
			SourceMdlAnimationDesc37 firstAnimDesc = theMdlFileData.theAnimationDescs[aSequenceDesc.theAnimDescIndexes[0]];
		...
		public void WriteBodyGroupCommand(int startIndex)
		{
			...
			//	blank
			//}
			if (theMdlFileData.theBodyParts != null && theMdlFileData.theBodyParts.Count > startIndex)
			{
				line = "";
				theOutputFileStreamWriter.WriteLine(line);

				for (int bodyPartIndex = startIndex; bodyPartIndex < theMdlFileData.theBodyParts.Count; bodyPartIndex++)
				{
