			...
			try
			{
				if (aBodyModel.theMeshes != null)
				{
					for (int meshIndex = 0; meshIndex < aBodyModel.theMeshes.Count; meshIndex++)
					{
						...
						aTexture = this.theMdlFileData.theTextures[materialIndex];
							...
							for (int stripsAndFansIndex = 0; stripsAndFansIndex < aMesh.theStripsAndFans.Count; stripsAndFansIndex++)
							{
								...
								if (aStripOrFan.theFacesAreStoredAsTriangleStrips)
								{
									for (int vertexInfoIndex = 0; vertexInfoIndex <= aStripOrFan.theVertexInfos.Count - 3; vertexInfoIndex++)
									{
										...
										{
											// even
											vertex1Line = this.GetVertexLine(aBodyModel, aStripOrFan.theVertexInfos[vertexInfoIndex], aTexture);
											vertex2Line = this.GetVertexLine(aBodyModel, aStripOrFan.theVertexInfos[vertexInfoIndex + 2], aTexture);
											vertex3Line = this.GetVertexLine(aBodyModel, aStripOrFan.theVertexInfos[vertexInfoIndex + 1], aTexture);
										}
										else
										{
											// odd
											vertex1Line = this.GetVertexLine(aBodyModel, aStripOrFan.theVertexInfos[vertexInfoIndex], aTexture);
											vertex2Line = this.GetVertexLine(aBodyModel, aStripOrFan.theVertexInfos[vertexInfoIndex + 1], aTexture);
											vertex3Line = this.GetVertexLine(aBodyModel, aStripOrFan.theVertexInfos[vertexInfoIndex + 2], aTexture);
										}
								...
								else
								{
									for (int groupIndex = 1; groupIndex <= aStripOrFan.theVertexInfos.Count - 2; groupIndex++)
									{
										...
										vertex1Line = this.GetVertexLine(aBodyModel, aStripOrFan.theVertexInfos[0], aTexture);
										vertex2Line = this.GetVertexLine(aBodyModel, aStripOrFan.theVertexInfos[groupIndex + 1], aTexture);
										vertex3Line = this.GetVertexLine(aBodyModel, aStripOrFan.theVertexInfos[groupIndex], aTexture);
...
#region Private Methods
		...
		private string GetVertexLine(SourceMdlModel10 aBodyModel, SourceMdlVertexInfo10 aVertexInfo, SourceMdlTexture10 aTexture)
		{
			...
			try
			{
				...
				SourceBoneTransform10 boneTransform = this.theMdlFileData.theBoneTransforms[boneIndex];
