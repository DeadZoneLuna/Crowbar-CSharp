...
#region Methods
		...
		public void ReadMdlHeader00(string logDescription)
		{
			...
			{
				this.theMdlFileData.theFileSeekLog.Add(fileOffsetStart, fileOffsetEnd, logDescription + " (Actual version: " + this.theMdlFileData.version.ToString() + "; override version: 52)");
			}
		}
		...
		public void ReadMdlHeader02(string logDescription)
		{
			...
			this.theMdlFileData.illumPositionAttachmentIndex = this.theInputFileReader.ReadInt32();
	...
	//						Dim sectionCount As Integer
							...
							//'      panim->numsections = (int)(panim->numframes / panim->sectionframes) + 2;
							//'NOTE: It is unclear why "+ 2" is used in studiomdl.
							var sectionCount = Convert.ToInt32(Math.Truncate(anAnimationDesc.frameCount / (double)anAnimationDesc.sectionFrameCount)) + 2;
	...
	//						Dim adjustedAnimOffset As Long
		...
		protected long ReadMdlIkRules(long animInputFileStreamPosition, SourceMdlAnimationDesc52 anAnimationDesc)
		{
			if (anAnimationDesc.ikRuleCount > 0)
			{
				...
				if (fileOffsetOfLastEndOfIkRuleExtraData > 0)
				{
			...
			if (this.theMdlFileData.localSequenceCount > 0)
			{
				...
				try
				{
					...
					for (int i = 0; i < this.theMdlFileData.localSequenceCount; i++)
					{
						...
						if (this.theMdlFileData.version == 49)
						{
